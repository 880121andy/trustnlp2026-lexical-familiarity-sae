...
ACL 2026 Experiment - SAE Feature Analysis

This script analyzes how Semantic Shift slang (e.g., "cook", "slay", "vibe")
is processed across all 48 layers in Gemma-3-12B-PT using Gemma Scope 2 SAEs.
...
def load_model(model_id: str = "google/gemma-3-12b-pt"):
...
def load_sae(layer: int, width: str = "16k", l0: str = "small") -> Optional[SAE]:
    ...
    try:
        sae, cfg_dict, sparsity = SAE.from_pretrained(
            release="gemma-scope-2-12b-pt-res-all",
...
def main():
    ...
    # For semantic shift:
    # df = pd.read_csv('data/genz_dataset_tagged.csv')
    # pairs = load_semantic_shift_pairs(df)

    # For neologism:
    # df = pd.read_csv('data/neologism_tagged.csv')
    # pairs = load_neologism_pairs(df)

    # For constructions:
    # df = pd.read_csv('data/constructions_tagged.csv')
    # pairs = load_construction_pairs(df)

    # For metaphors:
    # df = pd.read_csv('data/metaphor_baseline.csv')
    # pairs = load_metaphor_pairs(df)

    # For idioms:
    # df = pd.read_csv('data/idiom_baseline.csv')
    # pairs = load_idiom_pairs(df)

    # For literal paraphrases:
    df = pd.read_csv('data/literal_paraphrase_baseline.csv')
    pairs = load_literal_paraphrase_pairs(df)

    # For identical:
    # df = pd.read_csv('data/identical_baseline.csv')
